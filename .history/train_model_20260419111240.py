...
=========================================================
100% scikit-learn / numpy — NO TensorFlow, NO PyTorch, NO DLL issues.

Models trained:
  A) Random Forest Classifier    → volatility regime (High/Low)
     XAI: LIME, DiCE

  B) MLP Regressor (sklearn)     → next-day Close price
     XAI: SHAP, Ablation, Permutation Importance, Saliency Map

  C) BETA Surrogate Decision Tree → mimics MLP (from CNS_Phase_3)
...
# CNS_Phase_3 feature names for MLP price predictor (flat input)
...
def mlp_numerical_gradient(model, flat_x, eps=1e-4):
    """
    Numerical gradient of MLP output w.r.t. each of the 300 input dims.
    Returns array of shape (300,).
...
def train_and_save():
    ...
    # ══════════════════════════════════════════════════════════════════════
    # MODEL A — Random Forest Volatility Classifier
    # ══════════════════════════════════════════════════════════════════════
    ...
    # Save a sample of test data for XAI endpoints
    ...
    # ══════════════════════════════════════════════════════════════════════
    # MODEL C — BETA Surrogate Decision Tree  (from CNS_Phase_3)
    # ══════════════════════════════════════════════════════════════════════
    ...
    # Pre-compute global sensitivity + LRP for the test set
    ...
    all_grads = np.array(all_grads)   # (n_grad, 300)

    # Reshape → (n_grad, 60, 5) then aggregate per feature
    grads_3d  = all_grads.reshape(n_grad, TIME_STEP, len(OHLCV_COLS))

    # Global sensitivity = mean |grad| per feature (averaged over time + samples)
    global_sensitivity = np.abs(grads_3d).mean(axis=(0, 1)).tolist()   # (5,)

    # Global LRP proxy = mean |grad × input| per feature
    X_te_3d   = X_te_m[:n_grad].reshape(n_grad, TIME_STEP, len(OHLCV_COLS))
    grad_x_in = np.abs(grads_3d * X_te_3d).mean(axis=(0, 1)).tolist()  # (5,)
